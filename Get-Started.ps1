...
# STIG Compliance Automation Repository WorkFlow
    ...
    # Step 1 - Install DSC Modules on localhost 
        # Clone/download the repository and install dependant modules
        git clone "https://PubSec-CDM@dev.azure.com/PubSec-CDM/PubSec%20STIG%20Dev%20Project/_git/SCAR"
        $modules = Get-ChildItem -Path ".\Resources\Modules" 
        foreach ($module in $modules) 
        { 
            Write-Output "$module - Copying to local Powershell module path"
            Copy-Item -Path $Module.FullName -Destination "$env:SystemDrive\Program Files\WindowsPowerShell\Modules" -Force -Recurse
        }
