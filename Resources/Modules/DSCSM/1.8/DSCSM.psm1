...
function Remove-ScarData
{
    ...
    .SYNOPSIS
    Removes all existing artifacts the SCAR repository including Nodedata, Compiled DSC Scripts, MOFs, and STIG Checklists
    ...
    $nodeDataPath   = (Resolve-Path "$RootPath\Nodedata").Path
    ...
    $nodedatafiles  = Get-Childitem -Path "$nodeDataPath\" -Recurse | Where {$_.name -notlike "*.keep*"}
...
function Copy-DSCModules
{
    ...
    .EXAMPLE
    Example Copy-DSCModules -rootpath "C:\SCAR"
    ...
    Write-Output "`n`t`t$($jobs.count) Nodedata jobs completed. Receiving job output"
    Get-Job -ID $jobs | Wait-Job | Receive-Job
...
function Complete-Prereqs
{
    ...
    .SYNOPSIS
    The Complete-Prereqs function compiles and executes 3 other functions to complete the prereqruisites required for the SCAR Build to complete.
    ...
    the correct MaxEnevelopeSize as is required by SCAR.
    ...
    -SearchBase "CN=SCAR Target Machines,OU=_Domain Administration,DC=corp,DC=contoso,DC=com"
    ...
    .EXAMPLE
    complete-prereqs -rootpath "C:\Your Repo\SCAR" -searchbase "CN=SCAR Target Machines,OU=_Domain Administration,DC=corp,DC=contoso,DC=com" -domainname "corp.contoso" -forestname "com"
...
function New-ConfigData
{
    ...
    Example -NodeDataPath "C:\SCAR\NodeData"
    ...
    .EXAMPLE
    New-ConfigData -Rootpath "C:\SCAR" -SearchBase "CN=Servers,CN=Enterprise Management,DC=contoso,DC=com"
        ...
        if ($targetMachines.Count -gt 0)
        {
            ...
            foreach ($machine in $TargetMachines)
            {
                ...
                try
                {
                    ...
                    Write-Output "`tStarting Job ($currentMachineCount/$ouMachineCount) - Generate NodeData for $machine"
                }
                catch
                {
                    Write-Output "`tError - Unable to connect to $machine via WinRM."
                    continue
                }
                ...
                $job = Start-Job -Scriptblock {
                    ...
                    try
                    {
                        ...
                        Write-Output "`t`t$machine - Nodedata successfully generated."
                    }
                    catch
                    {
            ...
            Write-Output "`tJob creation for $($ou.name) nodedata is complete. Waiting on $($jobs.count) jobs to finish processing."
            if ($jobs.count -ge 1)
            {
                do
                {
                    ...
                    Write-Output "`t$($ou.Name) NodeData Status:`t$runningJobs Jobs Currently Processing`t$completedJobs/$($jobs.count) Jobs Completed"
                    Start-Sleep -Seconds 30
                }
                while ((Get-Job -ID $jobs).State -contains "Running")
                Write-Output "`t$($jobs.count) Nodedata jobs completed. Outputting Results."
                Get-Job -ID $jobs | Wait-Job | Receive-Job
            }
            else
            {
...
function Update-ScarRepo
{
    ...
    $modulePath = (Resolve-Path -Path "$RootPath\Resources\Modules").Path
    $modules = Get-Childitem $ModulePath | Where-Object name -notlike "DSCSM"

    # Update Dependent Modules
    Write-Output "`n`tRemoving Module Dependencies"
    foreach ($module in $modules)
    {
    ...
    foreach ($xccdf in $processedXccdfs)
    {
        ...
        switch -Wildcard ($xccdf.name)
        {
            ...
            "*Web Server*"          {$xccdfFolderName = "Web Server"}
        ...
        $manualCheckFilePath = "$manualCheckFolder\$($xccdfContent.DisaStig.StigId)-manualChecks.xml"
...
function Build-ScarRepo
{
    ...
    $dscsmRoot = Split-Path $dscsmPath -Parent
    $configZip = "$dscsmRoot\Configurations.zip"
    $wikiZip   = "$dscsmRoot\wiki.zip"

    Expand-Archive $configZip -DestinationPath $configPath
    $configScripts = Get-Childitem $configPath\Configurations
    $configScripts | Copy-Item -Destination $configPath -Force
    Remove-Item $configPath\Configurations -Force -Recurse -Confirm:$false

    Expand-Archive $wikiZip -DestinationPath $wikiPath
    $configScripts = Get-Childitem $wikiPath\Wiki
    $configScripts | Copy-Item -Destination $wikiPath -Force
    Remove-Item $wikiPath\Wiki -Force -Recurse -Confirm:$false

    Update-ScarRepo -RemoveBackup

    Write-Output "`n`tInstalling/Importing SCAR Modules"
    Copy-DscModules -LocalHost -Force
    Import-Module PowerSTIG -Force
    Import-Module DSCSM -Force

    Write-Output "STIG Compliance Automation Repository Build Complete."
    Write-Output "Run New-ConfigData to begin nodedata creation.`n`n"
}